				...
				<view class="uni-media-list btm_border" @tap="goPath(value.user_id)">
					...
					<view>
						<view class="badge-text">
							<text>{{value.text.replace('查看',$t('friend.apply_list.look')).replace('已添加',$t('friend.apply_list.had_add'))}}</text>
						</view>
						<view><text>&nbsp;</text></view>
					</view>
				</view>
...
<script>
	...
	// import mehaotian from  '../../components/mehaotian-search/mehaotian-search.vue'
										...
										if (res.confirm) {
											_this.$httpSend({
												...
												success(res) {
													...
													_data.localData('friend_apply_list',
														_this.friend_apply_list);
													uni.showToast({
														title: '删除成功!',
														icon: 'none',
													})
