						...
						<view class="uni-list-cell" hover-class="none" v-for="(item,index) of list.data"
							...
							<checkbox-group @change="change(item.user_id + '')">
								<label class="uni-list-cell uni-list-cell-pd">
									<view>
										<checkbox color="#02b300" :checked="item.other" :disabled="item.other"
											:value="item.user_id + ''" />
									</view>
									<view class="uni-media-list">
										<view class="uni-media-list-logo">
											<image :data-index="index" :src="staticPhoto + item.photo" :lazy-load="true"
												style="border-radius: 50%;" @error="imageError" />
										</view>
							...
							uni.showToast({
								complete: () => {
									setTimeout(() => {
										...
										if (_this.list_id != '0') {
											uni.redirectTo({
												url: '../chat/message?list_id=' + _this
													.list_id,
											});
										} else {
											uni.switchTab({
