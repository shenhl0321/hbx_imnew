    ...
    <view class="page">
        <view class="grid">
            <view class="grid-title">
                <text>{{$t('newAdd.anzhuangchajian')}}</text>
            </view>
        ...
        <view class="grid-content">
            <view class="grid-item" v-for="(item, index) in gridList" :key="index" @click="gridClick(item, index)">
                ...
                <view>
                    <button type="primary" @tap="installApp" class="button-buy" plain="true">{{$t('newAdd.anzhuang')}}</button>
                </view>
            </view>
        </view>
                    ...
                    <view class="box">
                        ...
                        <t-table border="2" border-color="#95b99e" :is-check="false" @change="change">
                            <t-tr font-size="center" color="#95b99e" align="left">
                                <t-th align="center">{{$t('newAdd.group_nickName')}}</t-th>
                                <t-th align="center">{{$t('newAdd.robotCout')}}</t-th>
                                <t-th align="center">{{$t('newAdd.guoqitime')}}</t-th>
                                <t-th align="center">{{$t('newAdd.handle')}}</t-th>
                            </t-tr>
                            <t-tr font-size="12" color="#5d6f61" align="right" v-for="item in tableList" :key="item.id">
                                ...
                                <t-td align="left">
                                    <button @click="goPath(item)" class="edit-but">{{$t('newAdd.setup')}}</button>
                                </t-td>
                            </t-tr>
                        </t-table>
                    </view>
    ...
    export default {
        data() {
            return {
                ...
                gridList: [
                    {
                        name: this.$t('newAdd.duolei_redbag'),
                ...
                popuMsg:this.$('newAdd.chajian_dialog'),
                imgUrl:'/static/theme/default//my/hongbao.png',
