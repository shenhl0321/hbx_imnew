<template>
	<view class="page">
		<view class="content">
			<view class="content-info" style="margin-top:20rpx;">
				<image src=""
					style="border: 1rpx solid gainsboro; border-radius: 50%; margin-left: 20rpx; margin-right: 20rpx;">
				</image>
				<text style="color: black;">{{$t('my.wallet.sfk.ewm_sk')}}</text>
			</view>
			<view class="content-main">
				<text class="content-main-info">{{$t('my.wallet.sfk.tips1')}}</text>
				...
				<view class="content-main-qrCode">
					<tki-qrcode ref="qrcode" :val="qrData" :onval="true" :size="300" :iconSize="65" unit="upx"
				...
				<view class="content-main-footer">
					<view @tap="goPath('set_money')">{{set?$t('my.wallet.sfk.set_money'):'清除金额'}}</view>
					<!-- #ifndef MP-WEIXIN -->
					<view @click="saveImgToLocal">
						{{$t('my.wallet.sfk.save_skm')}}
					</view>
					<!-- #endif -->
					...
					<button v-else hover-class="none" open-type="openSetting"
						@opensetting='handleSetting'>{{$t('my.wallet.sfk.save_skm')}}</button>
					<!-- #endif -->
				</view>
			</view>
		</view>
