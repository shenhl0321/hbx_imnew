<template>
	<view class="page" v-if="!showVedio">
		...
		<uni-popup ref="popup" type="img">
			<uni-popup-img :title="$t('newAdd.warm_notice')" :beforeClose="true" @close="close" content="testst" @confirm="confrim">
			</uni-popup-img>
		</uni-popup>
	</view>
					...
					success: (data) => {
						if (data.statusCode === 200) {
							...
							uni.saveFile({
								...
								success(res) {
									uni.showModal({
										title: _this.$t('newAdd.downloadSuccess'),
										content: _this.$t('newAdd.fileSaveTo') + res.savedFilePath,
										success: (res_) => {
