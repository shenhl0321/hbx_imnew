	...
	<view class="page">
		<uni-nav-bar leftText="" :statusBar="true" backgroundColor="transparent" :border="false">
			...
			<template v-slot:default v-if="!load_image">
				<view class="nav-bar">
					<text class="title">{{$t('my.invite.invite_code')}}</text>
				</view>
			</template>
			<template class="" v-slot:right v-if="!load_image">
				<view class="icon-center" @click="refreshData">
					<image class="refresh-icon" src="/static/theme/default/chat/2.png" mode="widthFix"></image>
				</view>
			</template>
		</uni-nav-bar>
	...
	export default {
		...
		onShow() {
			...
			uni.setNavigationBarTitle({
				title: this.$t('my.invite.invite_code')
			})
		},
		onLoad() {
		...
		computed: {
			...
			qrData() {
				...
				let route = '/#/pages/in/reg'
				let query = '?invite_code=' + this.my_data.id
				let url = origin + route + query
				return url;
			},
		},
		methods: {
