	...
	<view class="page" v-if="!showVedio">
		<view class="page-header">
			<view class="page-header-info">
				<image :src="myPhoto" style="border-radius: 50%;flex-shrink: 0;"></image>
				<view class="page-header-left">
					<view>{{$t('collect.scgs')}}：{{storge_info.user_storge_count}}/{{storge_info.max_count}}</view>
					<view>{{$t('collect.sckj')}}：{{storge_info.user_storge}}/{{storge_info.max_storge}}</view>
				</view>
			</view>
			<view class="page-header-right">
				<view class="page-header-right-view1"></view>
				<view class="page-header-right-view2" @tap="toDetail">{{$t('collect.detail')}}</view>
			</view>
		</view>
		<view class="page-content">
			<view class="footer-hr"></view>
			<view class="page-content-header">
				<view @tap="getDetail(0)">{{$t('collect.all')}}</view>
				<view>|</view>
				<view @tap="getDetail(4)">{{$t('collect.video')}}</view>
				<view>|</view>
				<view @tap="getDetail(3)">{{$t('collect.voice')}}</view>
				<view>|</view>
				<view @tap="getDetail(2)">{{$t('collect.image')}}</view>
				<view>|</view>
				<view @tap="getDetail(1)">{{$t('collect.word')}}</view>
				<view>|</view>
				<view @tap="getDetail(11)">{{$t('collect.file')}}</view>
				<view>|</view>
			</view>
