	...
	<view class="page hongbao-page">
		<view class="content">
			<view class="hongbao-header">
				<!-- 自定义导航栏 -->
				<view class="flex align-center" style="position: relative;display: flex;align-items: center;">
					<image @tap="backGo" src="../../../static/share/xiangzuojiantou(1).png"
						style="width: 35rpx; height: 35rpx; margin-left: 20rpx;" mode=""></image>
					<text
						style="font-size: 40rpx;color: aliceblue;position: absolute;left: 50%;top:50%;transform: translate(-50%,-50%);"
						class="fonts">{{$t('my.wallet.zdmx')}}</text>
				</view>
				...
				<view class="hongbao-header-view">
					<view class="header-top">
						<view class="top-left">
							<!-- <image :src="myPhoto" clsass="top-left-left"></image> -->
							<view class="top-left-right">
								<!-- <text class="hb-name">{{my_data.nickname}}</text> -->
								<view>
									<text style="font-size: 24upx;color:ghostwhite;margin-right:7px"
										@click="showTimeList">{{timeName}}</text>
									<text style="font-size: 10upx;color:#bebebe"
										class="iconfont-im icon-sanjiao"></text>
								</view>

							</view>
							<view class="button-in" @click="getHb(1)" :class="get==1?'active':''">
								{{$t('my.wallet.capital.income')}}
							</view>
							<view class="button-in" @click="getHb(2)" :class="get==2?'active':''">
								{{$t('my.wallet.capital.pay')}}
							</view>
						</view>
						<view class="top-right" v-if="0">
							<view><text>{{$t('my.wallet.capital.get_bonus')}}</text><text
									class="top-right-num">{{info.get_count}} {{$t('my.wallet.capital.a')}}</text></view>
							<view><text>{{$t('my.wallet.capital.best_bonus')}}</text><text
									class="top-right-num">{{info.best_count}} {{$t('my.wallet.capital.time')}}</text>
							</view>
						</view>

					</view>
					<view class="header-bottom">
						<view style="color: gainsboro;font-size: 35rpx;">
							{{$t('my.wallet.capital.total_amount')}}:
						</view>
						<text style="color: white;" class="hongbao-num">{{info.amount}}</text>
					</view>
				</view>
			</view>
		...
		<!-- <view class="hongbao-footer">
            <view @click="getHb(1)" :class="get==1?'active':''"><text>收入</text></view>
            <view  @click="getHb(2)" :class="get==2?'active':''"><text>支出</text></view>
        </view> -->
		...
		onLoad(options) {
			if ('type' in options) {
				...
				uni.setNavigationBarTitle({
					title: this.$t('my.wallet.capital.my_bonus_log')
				})
			}
		},
		onShow() {
		...
		methods: {
			...
			getCapital(type, cb) {
				...
				_get.getCapitalList({
					...
					_this.info.user_money = res.info.user_money
					if (type == 'down') {
						_this.list = res.data;
						if (cb != undefined) cb();
					} else {
