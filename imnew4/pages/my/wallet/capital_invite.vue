    ...
    <view class="page hongbao-page">
                ...
                <view class="hongbao-header-view">
                    <view class="header-top">
                        ...
                        <view class="top-right" v-if="0">
                            <view><text>{{$t('newAdd.')}}</text><text class="top-right-num">{{info.get_count + $t('newAdd.ge') }}</text></view>
                            <view><text>{{$t('newAdd.best_bonus')}}</text><text class="top-right-num">{{info.best_count + $t('newAdd.ci')}}</text></view>
                        </view>
                        <view class="top-right">
                            <view><text>{{$t('newAdd.money')}}</text><text class="top-right-num">{{info.user_money + $t('newAdd.yuan')}}</text></view>
                        </view>
                    </view>
                    <view class="header-bottom">
                        <text>{{$t('newAdd.total_amount')}}</text><text class="hongbao-num">{{info.amount}}</text>
                    </view>
                </view>
            </view>
            <view class="capital-list"
                ...
                <view class="capital-item"
                    ...
                    <view class="item-right">
                        ...
                        <text class="yue">
                            <text style="margin-right: 8rpx;">{{$t('newAdd.money')}}</text>
                            {{item.user_money}}
                        </text>
                    </view>
                </view>
        ...
        <uni-popup ref="popup2"
            ...
            <view class="but-view">
                ...
                <view class="clone-but" @click="clonePopup">{{$t('newAdd.cancel')}}</view>
            </view>
        </uni-popup>
    </view>
</template>
    ...
    export default {
        ...
        data () {
            return {
                ...
                timeName:this.$t('newAdd.three_month'),
                type: '12',
                timeList:[{id:4,name:this.$t('newAdd.three_month')},
				{id:1,name:this.$t('newAdd.month')},
				{id:2,name:this.$t('newAdd.last_month')},
				{id:3,name:this.$t('newAdd.last_last_month')}]
            }
        },
        onLoad (options) {
           
			uni.setNavigationBarTitle({
				title: this.$t('newAdd.shouyi')
			})
        ...
        methods: {
            ...
            getCapital (type, cb) {
                let _this = this;
                _get.getCapitalList({ page: this.page, type: 12,get:_this.get,time:_this.time }, function (res) {
