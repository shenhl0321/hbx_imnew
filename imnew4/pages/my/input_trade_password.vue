...
<script>
    ...
    export default {
        ...
        data() {
            return {
                title: this.$t('newAdd.p_again_input_trade_pass'),
                show_key: true,
                password:'',
                set_msg: this.$t('newAdd.p_again_input_trade_pass'),
            ...
            getPassword(n) {
                ...
                if(_this.page_params.password != n.password){
                   return uni.showToast({
                        title: this.$t('newAdd.two_pass_incorrect'),
                        duration:2000,
                        icon:'none',
                        success:function () {
                           setTimeout(function () {
                ...
                _get.updateUserTradePassword(_this.page_params,function (res) {
                    uni.showToast({
                        title: _this.$t('newAdd.handle_success'),
                        duration:2000,
                        icon:'none',
                        success:function () {
                            _this.$refs.payKeyboard.cleanNum();
