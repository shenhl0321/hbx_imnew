<template>
  <view id="moments"
    ...
    <view class="new_msg"
          @tap="goCircleChat"
          v-if="no_reader_msg">{{$t('newAdd.new_msg')}}</view>
      ...
      <view class="post_right">
        ...
        <view class="pay_view"
              v-if="post.can_pay == 1"> <button :disabled="post.can_pay_times == 0"
                  ...
                  size="mini"
				   @tap="open(index)"
                  type="primary">{{post.can_pay_times == 0 ? $t('newAdd.saled') : ''+post.pay_amount + $t('newAdd.yuan')}}</button></view>
    ...
    <view :class="['action_main animated faster',(action_menu ? 'bounceInDown' : 'bounceOutUp')]"
      ...
      <view class="action_base">
        ...
        <view class="action_item"
              @tap="goAction(0)">
          <text class="action_item_text">{{$t('newAdd.send_image_word')}}</text>
        </view>

        <view class="action_item"
              @tap="goAction(1)">
          <text class="action_item_text">{{$t('newAdd.send_video')}}</text>
        </view>
      </view>
    </view>
...
export default {
  ...
  data () {
    return {
      ...
      index: '',
      comment_index: '',
      input_placeholder: this.$t('newAdd.comment'), //占位内容
  ...
  onShow () {
    ...
    /** 监听最新数据 */
    uni.$on('data_circle_data_user', function (data) {
      console.log(1111)
      _this.posts = data;
    });
    /** 监听新的个人数据 */
  ...
  onReachBottom () { //监听上拉触底事件
    ...
    _get.getUserCircleList({
      ...
      if (data.data.length < 10) {
        this.loadMoreText = this.$t('newAdd.noMoreData');
        this.is_more = true;
      }
    });
  },
  ...
  methods: {
    ...
    pullDownRefresh () {
      ...
      console.log("_this.user_id",_this.user_id)
      _get.getUserCircleList({
        time: time,
        type: 0,
        user_id: _this.user_id,
