			...
			<view class="uni-list">
				...
				<checkbox-group @change="checkboxGroupChange">
					<view class="uni-list-cell" hover-class="none"
						:class="list.length -1 == index ? 'uni-list-cell-last' : ''" v-for="(item,index) in list"
						:key="item.chat_id">
						<uni-swipe-action :options="[ {text: $t('newAdd.remark')} ]" @tap="swipeAction(item.user_id + '')">
							<view class="uni-media-list">
								...
								<view class="uni-media-list-logo">
									...
									<image :src="photo(item.photo_path+'')" :lazy-load="true"
										style="border-radius: 10px;" />
								</view>
								<view class="uni-media-list-body">
									<view class="uni-list-cell-navigate">{{item.show_name}}</view>
								</view>
							</view>
						</uni-swipe-action>
					</view>
		...
		methods: {
			initData() {
				...
				uni.$on('data_chat_list', function(data) {
					...
					let _arr = [...data]
					let _arr1 = _arr.filter(item => item.type == 1)
					console.log("群聊数据", _arr1)
					_this.list = _arr1;
				});
				_get.getChatList();
			},
			...
			zhuan() {
				...
				if (this.ids.length > 0) {
					this.$httpSend({
						...
						success(res) {
							uni.showToast({
								title: _this.$t('newAdd.forward_success'),
								icon: 'none',
								ducation: 2000,
							});
							uni.navigateBack({
