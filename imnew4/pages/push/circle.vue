	...
	<view id="moments" class="page">
		<view v-if="showVedio" class="showVedio" :style="{height:screenHeight+'px',width:'100%'}">
			<!-- <video :src="videoPath"
             ...
             object-fit="fill"
             airplay="allow" /> -->
			<video autoplay="true" page-gesture="true" controls="false" @fullscreenchange="fullscreenchange"
				:style="{height:'100%',width:'100%'}" id="myVideo" class="showVedio-video" :src="videoPath">
		...
		<view class="home-pic">
			<avatar selWidth="600upx" selHeight="600upx" @upload="upload" :avatarSrc="circleImg"
			...
			<view class="home-pic-base">
				...
				<view class="top-name" style="color: #FFFFFF;margin-bottom: 10px;">{{ my_data.nickname }}</view>
			</view>
		</view>

		<view class="new_msg" @tap="goCircleChat" v-if="no_reader_msg">{{$t('push.circle.new_msg')}}</view>

		<view class="moments__post" v-for="(post,index) in posts" :key="index" :id="'post-'+index">
			<view class="post-left">
				<image class="post_header" :src="staticPhoto + post.header_image + '?_=' + imgRan"
					@tap="goDetails(post.uid)" :lazy-load="true" />
			</view>
			...
			<view class="post_right">
					...
					<view class="thumbnails" v-if="post.type == 0" @tap="init_input()">
						<view :class="post.content.value.length === 1 ? 'my-gallery' : 'thumbnail' "
							...
							<image class="gallery_img" :lazy-load="true" mode="aspectFill"
								:src="staticPath + post.post_id +'/' + image" :data-src="image"
								@tap="previewImage(index,index_images)" />
							<image v-if="post.content.video_path" @tap="previewImage(index,index_images)"
								class="bofang-button"
								style="width: 30px;height: 30px;position: absolute;top: 315rpx;left:220rpx"
								mode="aspectFill" src="/static/theme/default/my/bofang.png"></image>
						</view>
					</view>
				</view>
				<view class="pay_view" v-if="post.can_pay == 1"> <button :disabled="post.can_pay_times == 0"
						:style="{'background-color':post.can_pay_times == 1?'#4591ff':'#A9A9A9'}" size="mini"
						@tap="open(index)"
						type="primary">{{post.can_pay_times == 0 ? $t('push.circle.saled')  : ''+post.pay_amount + $t('push.circle.cny')}}<}}</button></view>

				<!-- 资料条 -->
				<view class="toolbar">
					<view class="timestamp">{{ timestampFormat(post.time+'')}}
						<text class="del-icon" v-if="post.uid == my_data.id" @tap="delSay(post.post_id)">{{$t('push.circle.delete')}}</text>
					</view>
					<view class="like" @tap="like(index)">
						<image
							:src="post.islike === 0 ? '../../static/theme/default/push/circle/islike.png' : '../../static/theme/default/push/circle/like.png'"
							:lazy-load="true" />
					</view>
				...
				<view class="post-footer">
					<view class="footer_content" v-if="post.like.length">
						<image class="liked" src="../../static/theme/default/push/circle/liked.png" :lazy-load="true">
						</image>
		...
		<view :class="['action_main animated faster',(action_menu ? 'bounceInDown' : 'bounceOutUp')]"
			...
			<view class="action_base">
				...
				<view class="action_item" @tap="goAction(0)">
					<text class="action_item_text">{{$t('push.circle.send_image_word')}}</text>
				</view>

				<view class="action_item" @tap="goAction(1)">
					<text class="action_item_text">{{$t('push.circle.send_video')}}</text>
				</view>
			</view>
		</view>
		<uni-popup ref="popup" type="address">
			<uni-popup-address :userAddress="userAddress" :amount="amount" :goods_name="goods_name"
				:small_pic="small_pic" @confirm="goBuy"></uni-popup-address>
		</uni-popup>
		<pay-keyboard :showCose="showCose" :set_info="set_info" :show_key="show_key" ref="payKeyboard"
			:set_msg="set_msg" @hideFun="hideFun" @getPassword="getPassword"></pay-keyboard>
	</view>
	...
	export default {
		...
		data() {
			return {
				...
				loadMoreText: this.$t('chat.message.index.loading') + "...",
				showLoadMore: false,
				is_more: false,
				no_reader_msg: 0,
				doubleClick: false,
		...
		computed: {
			...
			circleImg() {
				return _data.staticPhoto() + (this.my_data.circle_img?this.my_data.circle_img:'default_circle_img.jpg');
			},
			staticPath() {
		...
		onShow() {
			uni.setNavigationBarTitle({
				title:this.$t('push.index.circle')
			})
			_hook.routeSonHook();
			let _this = this;
			...
			uni.$on('data_circle_data', function(data) {
				console.log('data_circle_data:', data)
				// 过滤重复数据
				let obj = {}
				data.forEach(item=>{
				...
				let data_list = []
				for(let item in obj){
					data_list.push(obj[item])
				}
				console.log('朋友圈数据:', data)
				console.log('朋友圈数据:', data_list)
				_this.posts = data_list;
			});
			/** 监听新的个人数据 */
		...
		onUnload() {
			...
			this.max = 0,
				this.data = [],
				this.loadMoreText = this.$t('newAdd.loadmore'),
		...
		onReachBottom() { //监听上拉触底事件
			...
			_get.getCircleList({
				...
				if (data.data.length < 10) {
					this.loadMoreText = _this.$t('push.circle.nomore');
					this.is_more = true;
				}
			});
		},
		...
		onNavigationBarButtonTap(e) { //监听标题栏点击事件
			...
			if (e.index == 0) {
				...
				// this.action_menu = !this.action_menu
				let list = [{
						name: this.$t('push.circle.image_word'),
						key: "image"
					}, {
						name: this.$t('push.circle.video'),
						key: "video"
					},
					// {
					// 	name: "取消",
				...
				uni.showActionSheet({
					...
					success: res => {
						...
						uni.navigateTo({
							url: './circle/send?type=' + list[res.tapIndex].key + '&user_id=' + _this
								.user_id,
						});
					},
				});
		...
		onPageScroll(e) {
			...
			if (e.scrollTop < 45) {
				let tran = e.scrollTop / 44;
				this.setNavStyle(tran);
			} else {
				this.setNavStyle(1);
			}
		},
		methods: {
			...
			setNavStyle(tran) {
				// #ifdef H5
				// let btn = document.getElementsByClassName('uni-page-head-btn');
				// for (let i = 0; i < btn.length; i++) {
				// 	console.log("btn", btn);
				// 	btn[i].style.backgroundColor = 'rgba(0,0,0,0)!important';
				// }
				// console.log("tran", tran)
				if (tran > 0.5) {
					uni.setNavigationBarColor({
						backgroundColor: `rgba(255,255,255,${tran})`,
						animation: {
							duration: 300
						},
						frontColor: `#000000`,
					...
					uni.setNavigationBarTitle({
						title: this.$t('push.game.circle')
					})
				} else {
					uni.setNavigationBarColor({
						backgroundColor: `rgba(0,0,0,${tran})`,
						animation: {
							duration: 300
						},
						frontColor: `#ffffff`,
			...
			delSay(id) {
				let _this = this;
				uni.showModal({
					title: this.$t('newAdd.important_dialog'),
					content: this.$t('newAdd.certain_delete'),
					confirmText: this.$t('newAdd.delete'),
					cancelText: this.$t('newAdd.cancel'),
					confirmColor: 'red',
					cancelColor: '#353535',
							...
							_this.$httpSend({
								...
								success(data) {
									...
									if (data.code == 0) {
										uni.showToast({
											icon: 'none',
											title: _this.$t('newAdd.delete_success')
										})
										_this.posts = [];
			...
			reply(index, comment_index) {
				...
				this.input_placeholder = this.$t('push.circle.reply') + replyTo;
			...
			send_comment: function(message) {
				...
				if (this.is_reply) {
					...
					if (is_posts_obj.uid != chat_user_id) {
						reply = _this.$t('push.circle.reply') + is_reply_obj.username;
					}
				}
				_this.$httpSend({
			...
			init_input() {
				this.showInput = false;
				this.focus = false;
				this.input_placeholder = this.$t('push.circle.comment');
				this.is_reply = false;
			},
			...
			goBuy(address) {
				...
				_get.payCicleOrder(params, function(ret) {
					//更新本地朋友圈信息
					uni.showToast({
						title:  _this.$t('newAdd.buy_success'),
						icon: 'none',
					});
