	...
	<view class="page" @touchstart="touchStart" @touchend="touchEnd">
		<form>
			<view class="uni-textarea">
				<textarea :placeholder="$t('push.send.idea')" v-model="input_content" />
			</view>
			...
			<view v-if="type === 'video'" class="uni-list list-pd">
				<view class=" cell-pd">
					<view class="uni-uploader">
						<view class="uni-uploader-head">
							<view class="uni-uploader-title"></view>
							<view class="uni-uploader-info">{{$t('push.send.select_video')}}</view>
						</view>
						<view class="uni-uploader-body">
							<view class="uni-uploader__files">
								<block v-for="(image,index) in videoList" :key="index">
									<view class="uni-uploader__file" style="position: relative;">
										<video class="uni-uploader__img" mode="aspectFill" :src="image"
											:data-src="image" @tap="previewImage"></video>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.sizeType = [this.$t('newAdd.yasuo'), this.$t('newAdd.origal'), this.$t('newAdd.yasuo_origal')]
			this.sourceType = [this.$t('newAdd.shot'), this.$t('newAdd.images'), this.$t('newAdd.shot_images')]
			this.cameraList = [{
					value: 'back',
					name: this.$t('newAdd.back'),
					checked: 'true'
				},
				{
					value: 'front',
					name: this.$t('newAdd.front')
				}
			]
		...
		onShow() {
			uni.setNavigationBarTitle({
				title: this.$t('push.send.page_title')
			})
			_hook.routeSonHook();
		},
		onUnload() {
		...
		methods: {
			...
			publish() {
				if (!this.input_content) {
					uni.showModal({
						content: this.$t('push.send.no_more'),
						showCancel: false,
					});
					return;
				}
					...
					if (dataList.length) {
						for (var i = 0, len = dataList.length; i < len; i++) {
							_this.$httpSendFile({
								...
								fail() {
									_this.$httpSend({
										path: '/im/upload/deleteCircleFile'
									});
								},
							});
						}
					...
					_this.$httpSend({
						...
						success(res) {
							...
							if (_this.imageList.length || _this.videoList.length) {
								_this.$httpSend({
									path: '/im/upload/circleFileAction',
									data: {
			...
			isFullImg: function() {
				return new Promise((res) => {
					uni.showModal({
						content: this.$t('newAdd.image_over'),
						success: (e) => {
							if (e.confirm) {
								this.imageList = [];
								res(true);
