<template>
	<view id="moments" class="page">
		...
		<view class="home-pic">
			<image :src="circleImg" style="width: 750upx; height: 500upx; border-radius: 0upx;" />
			</image>
			...
			<view class="post_right">
				...
				<view class="toolbar">
					<view class="timestamp">{{ timestampFormat(post.time+'')}}
						<text class="del-icon" @tap="delSay(post.post_id)">{{$t('push.circle.delete')}}</text>
					</view>
...
<script>
	...
	export default {
		...
		onUnload() {
			...
			this.max = 0,
				this.data = [],
				this.loadMoreText = this.$t('push.circle.loadmore');
		...
		onNavigationBarButtonTap(e) { //监听标题栏点击事件
			...
			if (e.index == 0) {
				...
				// this.action_menu = !this.action_menu
				let list = [{
					name: this.$t('newAdd.image_word'),
					key: "image"
				}, {
					name: this.$t('newAdd.video'),
					key: "video"
				}, {
					name: this.$t('newAdd.cancel'),
					key: "cancle"
				}]
				uni.showActionSheet({
		...
		methods: {
			...
			delSay(id) {
				...
				uni.showModal({
					...
					content: _this.$t('push.circle.delete_circle'),
					confirmText: _this.$t('push.circle.delete'),
					cancelText: _this.$t('push.circle.cancel'),
					confirmColor: 'red',
					cancelColor: '#353535',
				...
				this.$httpSend({
					...
					success(data) {
						if (data.data.length) {
							...
							_this.loadMoreText = _this.$t('push.circle.nomore');
							_this.is_more = true;
						}
						_this.my_data = data.user_info;
					}
			...
			reply(index, comment_index) {
				...
				this.input_placeholder = this.$t('newAdd.reply') + replyTo;
			...
			send_comment: function(message) {
				...
				if (this.is_reply) {
					...
					if (is_posts_obj.uid != chat_user_id) {
						reply = this.$t('newAdd.reply') + is_reply_obj.username;
					}
				}
				_this.$httpSend({
			...
			init_input() {
				this.showInput = false;
				this.focus = false;
				this.input_placeholder = this.$t('newAdd.comment');
				this.is_reply = false;
			},
