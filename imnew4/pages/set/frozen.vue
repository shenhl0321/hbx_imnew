	...
	<view class="content">
		<view class="frozen-bg">
			<view class="frozen-card">
				...
				<view class="frozen-head">{{$t('newAdd.frozen_loss')}}</view>
				<view class="frozen-function">
					
						<text>{{$t('newAdd.frozen_ys')}}</text>
			...
			<view class="frozen-btn">
				<button type="primary" :class="['landing', 'landing_true' ]" @tap="subLongin">{{$t('newAdd.frozen_start')}}</button> 
				...
				_this.$httpSend({
					...
					success: (data) => {
						uni.setStorage({
							...
							fail: () => {
								uni.showModal({
									content: this.$t('newAdd.local_cache_cant_use'),
								});
							},
							success(){
								uni.reLaunch({
