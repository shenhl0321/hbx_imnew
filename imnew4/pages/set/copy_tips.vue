<template>
    <view class="page">
        <view class="page-header">
            <image :src="photo" :lazy-load="true" />
            <text class="page-header-title">{{info.name}}</text>
            <text>{{info.num}}人</text>
        </view>
        <view class="page-content">
            <view>{{$t('newAdd.warm_notice')}}</view>
            <view>{{$t('newAdd.copytips_one')}}</view>
            <view>{{$t('newAdd.copytips_two')}}</view>
        </view>
        <view class="page-footer">
            <view class="page-footer-info" @tap="change">
                <checkbox :checked="info.checked"></checkbox>
                <text>{{$t('newAdd.over_group')}}</text>
            </view>
            <view>
                <button class="sub-but" style="background-color: #496cb1" @tap="copyQun">{{$t('newAdd.certainCopy')}}</button>
            </view>
        </view>
    </view>
</template>
