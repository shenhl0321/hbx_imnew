<template>
	<view class="page">
		...
		<scroll-view class="scrollList" scroll-y :scroll-into-view="scrollViewId" :style="{height:winHeight + 'px'}">
			...
			<view class="uni-list">
				...
				<block v-for="(list, key) in list_data" :key="key">
					...
					<view class="uni-list-cell" hover-class="none" v-for="(item,index) of list.data"
						:class="list.data.length -1 == index ? 'uni-list-cell-last' : ''" :key="isKey(key,index)">

						<checkbox-group @change="change">
							<label class="uni-list-cell uni-list-cell-pd">
								<view>
									<checkbox color="#02b300" :checked="current == item.user_id"
										:value="item.user_id + ''" />
								</view>
		...
		<view class="uni-indexed-list-bar" :class="touchmove ? 'active' : ''" @touchstart="touchStart"
			@touchmove="touchMove" @touchend="touchEnd" @touchcancel="touchCancel" :style="{height:winHeight + 'px'}">
			<text v-for="(list,key) in list_data" :key="key" class="uni-indexed-list-text"
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			if (_this.type == 2) {
				uni.setNavigationBarTitle({
					title: this.$t('g_set.jy')
				});
			} else {
				uni.setNavigationBarTitle({
					title: this.$t('g_set.copy')
				})
			}
			_this.$httpSend({
		...
		onNavigationBarButtonTap(e) {
			...
			uni.navigateTo({
				url: '../set/copy_tips?main_id=' + trans_user_id + '&list_id=' + _this.list_id
			})
			// _this.$httpSend({
