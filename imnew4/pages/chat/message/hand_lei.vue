	...
	<view>
		...
		<view class="content" :class="typeClass">
			<view class="luck">
				...
				<view class="row">
					<view class="term">
						{{$t('newAdd.bonus_boom_value')}}
					</view>
					<view class="input">
						<input type="number" disabled="true" :placeholder="$t('newAdd.select_boom_value')" v-if="!lei.length"/>
		...
		methods:{
			...
			hand(type){
				...
				if(_this.lei.length <= 0){
                    return uni.showToast({title:this.$t('newAdd.select_redbag_boom'),icon:'none'});
                }
				//判断数据有效性
				...
				redEnvelopeData.hongbao_msg = JSON.stringify(redEnvelopeData.hongbao_msg)||this.$t('newAdd.congratulations');//没填写则默认恭喜发财
