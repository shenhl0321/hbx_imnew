	...
	<view>
		<view class="top">
			...
			<view class="face" :style="{'border-radius':radius}">
				<image :src="own_user_info.face_new" :lazy-load="true"></image>
			</view>
			<view class="username">
				{{own_user_info.nickname}}
			</view>
		</view>
		<view class="info">
			{{$t('chat.message.received')}}{{hong_bao_detail.number - hong_bao_detail.sy_number}}/{{hong_bao_detail.number }}{{$t('chat.message.a')}}，{{$t('chat.message.total')}}{{(hong_bao_detail.money - hong_bao_detail.sy_money).toFixed(2)}}/{{hong_bao_detail.money}}{{$t('pay.cny')}}
		</view>
		<view class="list">
	...
	export default {
		...
		onShow(){
			...
			uni.setNavigationBarTitle({
				title:this.$t('chat.message.red_bag')
			})
			_hook.routeSonHook();
			_get.getHongBaoDetail(_this.pageParams,function (res) {
				_this.list = res.list;
				if(res.is_niming == 1){
					res.own_user_info.nickname = _this.$t('chat.message.anonymous');
					res.own_user_info.face_new = './static/theme/default/my/niming.png';
				}else{
					res.own_user_info.face_new = _data.staticPhoto() + res.own_user_info.face;
				}
