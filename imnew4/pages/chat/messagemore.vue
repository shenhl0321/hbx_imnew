<template>
                  ...
                  <image src="/static/img/red-envelope-chat.png"
						 ...
						 <block v-if="row.msg.content.rid_user_id == my_data.id && row.msg.content.user_id == my_data.id">
						        ...
						        <block v-else-if="row.msg.content.rid_user_id != my_data.id && row.msg.content.user_id == my_data.id">
						         {{$t('newAdd.i_got') + row.msg.content.rid_nickname + $t('newAdd.issued')}}
						        </block>
						        <block v-else-if="row.msg.content.rid_user_id == my_data.id && row.msg.content.user_id != my_data.id">
						          {{row.msg.content.nickname + $t('newAdd.got_i_issued')}}
						        </block>
						        <block v-else>
						          {{row.msg.content.nickname + $t('newAdd.got') + row.msg.content.rid_nickname + $t('newAdd.issued')}}
						        </block>
                  <text style="color: red;"
                        @tap="toDetails(row.msg.content.rid)">{{$t('newAdd.red_bag')}}</text>
                </view>
              </view >
            </block>
              ...
              <view class="my"
                ...
                <view class="left"
                  ...
                  <view v-if="row.msg.type==5"
                    ...
                    <view class="tis">
                      {{(receive_list.indexOf(row.msg.content.rid)) <0?$t('newAdd.congratulations_on_making_a_fortune'):$t('newAdd.received')}}
                    </view>
                    <view class="blessing">
                        <view class="info">
  ...
  <swiper class="more-layer" style="height:50vw" :indicator-dots="true" indicator-active-color="#51c9c3"
  		...
  		<swiper-item>
  			<view class="list">
				...
				<view class="list-box" @tap="chooseCard">
					...
					<view class="tool_text2">{{$t('chat.message.business_card')}}</view>
				</view>
				...
				<view class="list-box" @tap="sendPos">
					...
					<view class="tool_text2">{{$t('chat.message.location')}}</view>
				</view>
  
  				<!-- <view class="list-box" @tap="video">
  					<image class="fun-icon" src="../../static/theme/default/camera.png" mode="heightFix">
  					</image>
    ...
    <view class="windows" :class="windowsState">
      ...
      <view class="layer" @touchmove.stop.prevent="discard">
        <view class="open-redenvelope">
          ...
          <view class="from">{{$t('newAdd.from') + redenvelopeData.from + $t('newAdd.isRedbag')}}</view>
    ...
    <uni-popup ref="popup" type="confirm">
      <uni-popup-confirm :title="$t('newAdd.warm_notice')" :content="popuMsg" @confirm = "confirm"> </uni-popup-confirm>
    </uni-popup>
    <uni-popup ref="qiang" type="qiang">
      <uni-popup-qiang :title="$t('newAdd.open_bonus')" :status="miaoqiang + ''" :content="popuMsg" @confirm = "goQiang"> </uni-popup-qiang>
    </uni-popup>
  </view>
</template>
<script>
  ...
  export default {
    ...
    onLoad (option) {
      this.list_id = option.ids;
	  this.niming_msg = this.$t("newAdd.niming")
	  this.voiceTis = this.$t("newAdd.enterToSay")
	  this.recordTis = this.$t("newAdd.upCancelSend")
    ...
    methods: {
      ...
      getGroupSteate(){
        ...
        this.$httpSend({
          ...
          success (res) {
            ...
            _this.niming_msg = res.is_niming == 0 ? this.$t('newAdd.niming') : this.$t('newAdd.logout_niming');
      ...
      videoCall (type) {
        ...
        _get.getVedioPayConfig({}, function (ret) {
          let msg1 = this.$t('newAdd.online_video_call_per_minute') + ret.vedio + this.$t('newAdd.yuan');
          let msg2 = this.$t('newAdd.online_vocie_call_per_minute') + ret.voice + this.$t('newAdd.yuan');
          _this.popuMsg = msg1;
          let amount = _this.type == 1 ?  ret.vedio : ret.voice;
          if(_this.my_data.money < amount) {
            uni.showToast({
              title:this.$t('newAdd.moneyNoPayOneSec')+ _this.type == 1 ? this.$t('newAdd.video_talk') : this.$t('newAdd.voice_talk'),
              duration:3000,
              icon:"none"
            })
            return false;
      ...
      msgAction (id, user_id) {
        ...
        // }
        uni.showActionSheet({
          itemList: [this.$t('newAdd.back'),this.$t('newAdd.collect'),this.$t('newAdd.trans')],
          success: function (res) {
            switch (res.tapIndex) {
              case 0:
                _this.$httpSend({
              ...
              case 1:
				...
				case 2:
				      console.log("转发");
				       let datas ={ list_id: _this.list_id, type: _this.msgList.type, msg_id: id };
      ...
      goDetails (user_id,row) {
        ...
        if (this.msgList.type == 1 && (this.msgList.is_action == 0 || row.msg.is_niming == 1)) {
          uni.showToast({
            title: this.$t('newAdd.noAuthToSee'),
            icon: 'none',
            duration: 1000
          });
          return;
      ...
      goQiang(e){
        ...
        e.list_id = _this.list_id;
          _get.updateMiaoqiang(e,function (res) {
            ...
            setTimeout(function () {
              uni.showToast({
                title: this.$t('newAdd.handle_success'),
                duration: 2000
              });
      ...
      sendMsg (content, type) {
        ...
        ((callback) => {
          ...
          switch (type) {
            ...
            case 3:
            case 4:
              uni.showLoading({ title: this.$t('newAdd.sending') })
              _this.$httpSendFile({
                type: 0,
          ...
          _this.$httpSend({
            ...
            success (res) {
				uni.showToast({
					title:'Gửi hàng loạt thành công',
					icon:'none',
					ducation:2000,
				});
				uni.switchTab({
              ...
              switch (res.err) {
                ...
                case 1:
                  uni.showModal({
                    title: this.$('newAdd.friend_dialog'),
                    confirmText: this.$('newAdd.send_friend_apply'),
                    content: res.msg,
                    success: function (res) {
                      if (res.confirm) {
                        uni.navigateTo({
      ...
      openRed (params) {
        ...
        uni.showLoading({
          title: _this.$t('chat.message.index.loading') + '...'
        });
        //TODO 判断是哪种红包
      ...
      recordBegin (e) {
        ...
        this.AUDIO.pause();
        this.recording = true;
        this.voiceTis = this.$t('newAdd.release_end');
      },
      // 录音被打断
      voiceCancel () {
        this.recording = false;
        this.voiceTis = this.$t('newAdd.hold_to_talk');
        this.recordTis = this.$t('newAdd.slide_your_finger_up_to_cancel_sending');
      ...
      voiceIng (e) {
        ...
        if (this.initPoint.Y - touche.clientY >= uni.upx2px(100)) {
          this.willStop = true;
          this.recordTis =this.$t('newAdd.release_finger_to_cancel_sending');
        } else {
          this.willStop = false;
          this.recordTis = this.$t('newAdd.slide_your_finger_up_to_cancel_sending');
        }
      ...
      sayCall(){
        ...
        if(this.msgList.is_niming == 0){
          uni.showLoading({'title':this.$t('newAdd.niming_open')});
          status = 1;
        }
        let _this = this;
        _get.updateUserSayType({list_id:this.list_id,status:status},function () {
          if(_this.msgList.is_niming == 0){
            uni.showToast({'title':this.$t('newAdd.open_success'),'image':'/static/theme/default/my/niming.png',duration:2000});
            _this.msgList.is_niming = 1;

            _this.niming_msg = this.$t('newAdd.logout_niming');
          }else {
            _this.msgList.is_niming = 0;
            _this.niming_msg = this.$t('newAdd.niming');
          }
      ...
      getServing(){
          ...
          if (this.msgList.type == 1 && (this.msgList.is_action == 0) &&  this.group_state && this.group_state.can_shangmai == 0) {
              uni.showToast({
                  title: this.$t('newAdd.group_manager_on_microphone'),
                  icon: 'none',
                  duration: 1000
              });
              return;
