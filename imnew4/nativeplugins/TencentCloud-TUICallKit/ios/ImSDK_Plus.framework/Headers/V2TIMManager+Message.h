...
typedef NS_ENUM(NSInteger, V2TIMGroupInfoChangeType){
    ...
    V2TIM_GROUP_INFO_CHANGE_TYPE_GROUP_ADD_OPT       = 0x0B,  ///< 加群选项变更
};
...
/**
 *  5.15 根据 messageID 查询指定会话中的本地消息
...
/**
 * 5.16 搜索本地消息（5.4.666 及以上版本支持，需要您购买旗舰版套餐）
...
/**
 * 5.19 获取群消息已读群成员列表（6.1 及其以上版本支持）
 * @param message 群消息
...
/**
 * 5.20 设置消息扩展（6.7 及其以上版本支持，需要您购买旗舰版套餐）
 ...
 *  @param target 目标语言。支持的目标语言有多种，例如：英语-“en“，简体中文-”zh“，法语-”fr“，德语-”de“等。
...
/// 根据变更类型表示不同的值, 当前只有 type = V2TIM_GROUP_INFO_CHANGE_TYPE_RECEIVE_MESSAGE_OPT 和 V2TIM_GROUP_INFO_CHANGE_TYPE_GROUP_ADD_OPT 时有效 (从 6.5 版本开始支持)
...
//
/// 消息扩展信息
@interface V2TIMMessageExtension : NSObject
...
/// 消息扩展操作结果
@interface V2TIMMessageExtensionResult : NSObject
/// 返回码
