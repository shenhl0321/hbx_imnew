...
export default {
	...
	offline(res) {
		uni.showModal({
			...
			content:i18n.t('js.socket.again_login'),
			success() {
				_action.checkFail();
			},
		});
	...
	chatData(data) {
		...
		// 如果消息是视频通话类型，全局通知
		if (data.data.type == 0 && data.data.msg.content && data.data.msg.content.room ) {
			 // 判断是不是自己发出的消息
			 if(data.data.msg.user_info.uid != _data.data("user_info").id){
				 if (data.data.msg.content.invite) {
				 	uni.$emit("video_invite", data)
				 } 
				 if (data.data.msg.content.refuse) {
				 	uni.$emit("video_refuse", data)
				 }
				 if(data.data.msg.content.leave){
				 	uni.$emit("video_leave", data)
				 }
			 }
		}
		...
		for (let i = 0, local_chat_list = _data.localData('chat_list'), j = local_chat_list.length; i < j; i++) {
			if (local_chat_list[i].list_id == data.list_id) {
				switch (data.data.msg.type * 1) {
					...
					case 15:
						/** 名片 */
						local_chat_list[i].last_msg = '[视频通话]';
						break;
					default:
						/** 未知消息类型 */
