...
const pushLocalMsg = (data) => {
	...
	//#ifdef APP-PLUS
	let msg = '[unknow]';
	let nickname = data.data.msg.user_info.nickname || 'CVC TEAM';
	let is_niming = data.data.msg.is_niming || 0;
	if (is_niming == 1) {
		nickname = i18n.t('chat.message.anonymous')
	}
	switch (data.data.msg.type * 1) {
		...
		case 6:
			/** 在线视频 */
			msg = 'Cuộc gọi âm thanh và Video';
			break;
		case 7:
			/** 在线语音 */
			msg = 'Cuộc gọi âm thanh và Video';
